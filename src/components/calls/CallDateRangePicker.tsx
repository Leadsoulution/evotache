...
// Mirrors MetaDateRangePicker's design (src/components/socialmedia/ads/MetaDateRangePicker.tsx)
// — same left preset list + two-month calendar + Annuler/Mettre à jour footer
// — but with call-log-appropriate presets (windows ending today, not
// yesterday like Meta's ad-reporting presets) and a plain {from, to} ISO
// date range instead of Meta's date_preset API shape.
...
interface CallDateRangePickerProps {
  open: boolean;
  onClose: () => void;
  onApply: (range: CallDateRange, label: string) => void;
}

export function CallDateRangePicker({ open, onClose, onApply }: CallDateRangePickerProps) {
  const now = new Date();
