...
// Mirrors CallTimeRangePicker (src/components/calls/CallTimeRangePicker.tsx)
// — same modal chrome, same real opening-hours preset (relevant here too,
// since this filters actual attendance punches).
...
// midday break — so it's filtered by isWithinBusinessHours (BiometricView.tsx)
...
export interface BiometricTimeRange {
  from: string;
  to: string;
}

interface BiometricTimeRangePickerProps {
  open: boolean;
  onClose: () => void;
  onApply: (range: BiometricTimeRange, label: string, businessHours?: boolean) => void;
}

export function BiometricTimeRangePicker({ open, onClose, onApply }: BiometricTimeRangePickerProps) {
