...
import type { PurchaseColumnDef, PurchaseDropdownOption } from "@/types/purchase";

interface PurchaseColumnHeaderProps {
  column: PurchaseColumnDef;
  typeLabel: string;
  accentColor: string;
  onRename: (name: string) => void;
  onSetOptions: (options: PurchaseDropdownOption[]) => void;
  onDelete: () => void;
}

export function PurchaseColumnHeader({ column, typeLabel, accentColor, onRename, onSetOptions, onDelete }: PurchaseColumnHeaderProps) {
    ...
    <>
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          <span className="block truncate text-xs font-semibold uppercase tracking-wide text-slate-600 dark:text-slate-300">{column.name}</span>
          ...
          >
            ...
            <input
              ...
              onBlur={(event) => {
                ...
                else event.target.value = column.name;
              }}
            ...
            {column.type === "dropdown" && (
              <div className="mb-3">
                ...
                <DropdownOptionsEditor options={column.options} onChange={onSetOptions} />
              </div>
            )}
      ...
      <ConfirmDialog
        ...
        description={`"${column.name}" and its values on every row will be removed. This cannot be undone.`}
        confirmLabel="Delete"
        destructive
        onConfirm={() => {
          onDelete();
