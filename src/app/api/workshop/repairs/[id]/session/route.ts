...
// time instead of anything resetting to zero client-side.
export async function PATCH(request: Request, { params }: RouteContext) {
  ...
  const latest = await db.workshopSession.findFirst({ where: { repairId: id }, orderBy: { startedAt: "desc" } });
  const now = new Date();

  if (action === "start") {
    if (latest && !latest.endedAt) return NextResponse.json({ error: "A session is already active for this repair." }, { status: 409 });
    await db.workshopSession.create({
      data: { repairId: id, mechanicId: repair.mechanicId ?? sessionUser.id, startedAt: now, runningSince: now },
    });
    if (repair.status !== "in_progress") {
      await db.workshopStatusHistory.create({ data: { repairId: id, oldStatus: repair.status, newStatus: "in_progress", changedBy: sessionUser.id } });
      await db.workshopRepair.update({ where: { id }, data: { status: "in_progress" } });
    }
  ...
  const updatedRepair = await db.workshopRepair.findUnique({ where: { id } });
  const updatedSession = await db.workshopSession.findFirst({ where: { repairId: id }, orderBy: { startedAt: "desc" } });
  return NextResponse.json(toPublicWorkshopRepair(updatedRepair!, updatedSession));
}
